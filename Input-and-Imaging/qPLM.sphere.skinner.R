...
qPLM.sphere.skinner<-function(pixel.size, theta.max) {
  require(EBImage)
  require(colorspace)
  require(bmp)
  ...
  writeImage(output,file=paste(pixel.size, "_by_", theta.max, "_pixel_skin.tif", sep=""), bits.per.sample=8L, type="tiff")
