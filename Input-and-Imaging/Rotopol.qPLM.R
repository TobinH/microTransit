...
Rotopol.qPLM<-function(sample.name,thickness, wavelength, birefringence, pixel=7.5832259, up=0, mask=FALSE, pics.out=TRUE, comp.bkgrnd=50) {
  ...
  if (pics.out) {
    ...
    masked<-masked[(min(dindx[,1])-10):(max(dindx[,1])+10),(min(dindx[,2])-10):(max(dindx[,2])+10),]
    setWinProgressBar(pb, 14, title="Rotopol.qPLM: writing image: _trans")
    trans<-Image(Rotopol.distilled[(min(dindx[,1])-10):(max(dindx[,1])+10),(min(dindx[,2])-10):(max(dindx[,2])+10),1],colormode="Grayscale")
    ...
    theta<-Image(Rotopol.distilled[(min(dindx[,1])-10):(max(dindx[,1])+10),(min(dindx[,2])-10):(max(dindx[,2])+10),2],colormode="Grayscale")
    ...
    phi<-Image(Rotopol.distilled[(min(dindx[,1])-10):(max(dindx[,1])+10),(min(dindx[,2])-10):(max(dindx[,2])+10),3],colormode="Grayscale")
