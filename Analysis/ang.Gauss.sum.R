...
# arguments:
#   X: Euclidean coordinates for axial orientations (columns 3-5 of a qPLMtab object)
#   alpha: alpha value corresponding to confidence interval (1-CI). Default is 95% CI
#   tol: tolerance for convergence of iterative algorithm. Default value is the default for the all.equal() function.
#         decrease this value if the function returns an identity matrix for the lambda estimate
ang.Gauss.summ<-function(X, alpha=0.05, tol=sqrt(.Machine$double.eps)){
  ...
  convCrit2<- 0 # container for current interations's value of ""
