mech.mod.plot<-function(qPLMtab,mech.mod){
  require(ade4)
  oldpar<-par(no.readonly=TRUE)
  par(mfrow=c(1,3))
  sph<-matrix(data=0,nrow=nrow(mech.mod$mod.points),ncol=2)
  sph[,1]<-atan(mech.mod$mod.points[,1]/mech.mod$mod.points[,2])
  sph[,2]<-acos(mech.mod$mod.points[,3])
  ...
  s.value.tweak<-function (dfxy, z, xax = 1, yax = 2, method = c("squaresize", 
                           ...
                           clegend = 0.75, neig = NULL, cneig = 1, xlim = NULL, ylim = c(max(dfxy[,2]),min(dfxy[,2])), 
                           ...
                           pixmap = NULL, contour = NULL, area = NULL, add.plot = FALSE) 
  {
    ...
    if (method == "greylevel") {
      ...
      for (i in 1:(nrow(dfxy))) {
        symbols(coo$x[i], coo$y[i], squares = h, fg = gray(valgris[numclass[i]]), bg = gray(valgris[numclass[i]]), 
